...
export class PageEditComponent implements OnInit {
  ...
  validate_access() {
    ...
    this.http
      .post(environment.api_routes.validate_access_page, formData, {
      ...
      .then((response: any) => {
        ...
        if (response.data.access_guard != 'true') this.hasAccess = false;
      });
  }
  ...
  save_code() {
    ...
    const final_htmlCode: string = String(this.codeHTML).replace(
        '<!-- HTML Editor -->',
        ''
      ),
      final_js: string = String(this.codeJS).replace(
        '//JavaScript editor ',
        ''
      );
    ...
    this.http
      .post(environment.api_routes.save_page, formData, {
        withCredentials: true,
      })
      .toPromise()
      .then((response: any) => this.notifS.sendSuccess(response.message))
      .catch((error: any) => {
        let errors: string = '';
  ...
  async fetchCode() {
    ...
    formData.append('appID', this.appID);
    this.http
      .post(`${environment.api_routes.get_page_details}`, formData, {
        withCredentials: true,
      })
      .toPromise()
      .then((response: any) => {
